...
- **Fünf Minuten Vorlauf vor der Freischaltung, und der Wecker wiederholt sich** — bei
  ...
  Woche zu weit.
- **Der wiederkehrende Wecker stellt sich selbst neu, statt `setInexactRepeating` zu
  benutzen** — beim Zusammenführen mit `main` traf die Wiederholung auf `setAndAllowWhileIdle`
  (Zeile weiter unten). Beides zusammen geht nicht: Einen doze-festen *Wiederhol*wecker
  kennt Android nicht, und `setInexactRepeating` ist genau der Typ, den Doze bis zum
  Wartungsfenster liegen lässt. Bei fünf Minuten Vorlauf auf eine Freischaltung wäre das
  wertlos. Deshalb steht immer nur der nächste Termin, und der Empfänger stellt beim
  Auslösen den übernächsten — zuerst, noch vor dem Anzeigen, damit die Kette auch dann
  nicht abreißt, wenn das Melden an einer entzogenen Erlaubnis scheitert.
